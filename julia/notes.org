...
** TODO persistence
*** TODO integrate with reading tensorboard logs

**** Reading from python: table and plot design


**** references
- The [[https://www.tensorflow.org/tensorboard/migrate][migrate guide from v1 to v2]] should be a good quick resource.
- [[https://github.com/tensorflow/tensorboard][tensorflow/tensorboard]]

**** Reading back in Julia
[[https://github.com/PhilipVinc/TensorBoardLogger.jl][PhilipVinc/TensorBoardLogger.jl]] supports [[https://philipvinc.github.io/TensorBoardLogger.jl/dev/deserialization/][deserialization]], using
[[https://github.com/JuliaML/ValueHistories.jl][JuliaML/ValueHistories.jl]]. Example:
...
But it can NOT directly read a tf.event file. The creation options for TBLogger
are:
- =tb_increment=: default, create a new folder
- =tb_overwrite=: delete and overwrite
- =tb_append=: create a new FILE in the folder

Thus none of them reads back.


**** TODO as tensorboard plugins

So the data visualization should be tightly integrated with
tensorboard. Manually write matplotlib is painful, let alone extracting from
tensorboard event logs.

The current tensorboard front-end is not ideal because:
- download data button does not download all data, but a single run. Then you
  need to click around a lot.
- the image is not ideal: it lacks legends, the color assignment cannot be
  changed, and it turns out to conflict a lot

I need a plugin that supports:
- better color assignment
- show legends

That's not a lot. Let's see how tensorboard plugin works.
- [[https://github.com/tensorflow/tensorboard/blob/master/ADDING_A_PLUGIN.md][ADDING_A_PLUGIN.md]]: this is the official tutorial
- [[https://github.com/tensorflow/tensorboard/tree/master/tensorboard/examples/plugins/example_basic][tensorboard/examples/plugins/example_basic]]: an example (HEBI: TODO)
- [[https://github.com/tensorflow/tensorboard/tree/master/tensorboard/examples/plugins/example_raw_scalars][tensorboard/examples/plugins/example_raw_scalars]]: another more interesting
  example (still no figures) (HEBI: TODO)


How Tensorboard discovers plugins? looks like you install the plugin, and it
discovers itself?

Looks like we need the following pieces:
- optional data writer. In my case I just use the default writer (in Julia)
- post-process the data in backend in python: maybe this is optional as well?
  But I probably need to:
  - read tf event data here
  - pass the data to the front-end
- render it in JS: maybe I just draw the figure in JS?
  - looks like I have to draw figure in JS

About front-end:
#+begin_quote
TensorBoard does not impose any framework/tool requirements for building a
frontend—you can use React, Vue.js, jQuery, DOM API, or any new famous
frameworks and use, for example, Webpack to create a JavaScript
bundle. TensorBoard only requires an [[https://hacks.mozilla.org/2018/03/es-modules-a-cartoon-deep-dive/][ES Module]] that is an entry point to your
frontend (example ES module). Do note that all frontend resources have to be
served by the plugin backend (example backend).
#+end_quote

So (HEBI: TODO):
- react
- vue.js
- new [[https://webpack.js.org/][Webpack]]?
- jquery
- raw DOM

I'll write typescript of course.

Probably the next step is to read the source of tensorboard, see how the default
scalar figures are drawn. (HEBI: TODO)

**** Other tensorboard related packages:

- [[https://github.com/lanpa/tensorboardX][lanpa/tensorboardX]]: log tensorboard via pytorch (and others). This does not
  read back.
- The what-if-tool (WIT) is pretty complex:
  - on tensorboard page: https://www.tensorflow.org/tensorboard/what_if_tool
  - web: https://pair-code.github.io/what-if-tool/
  - source: https://github.com/pair-code/what-if-tool
  This seems not a tensorboard plugin, but a separate app, it consumes a tf
  Serving saved model and TFRecord dataset.

- [[https://github.com/tensorflow/tensorboard/tree/master/tensorboard/plugins/custom_scalar][custom_scalar plugin]]: progmatically specify the *layout* (see [[https://github.com/tensorflow/tensorboard/blob/799c5cf6ae1f5f8d78a09382689dcb9d0ec5f561/tensorboard/plugins/custom_scalar/custom_scalar_demo.py#L67-L117][this example
  code]]). Thus not very useful for me.

**** Read tensorboard event logs
Looks like pretty easy.

- [[https://www.tensorflow.org/versions/r1.15/api_docs/python/tf/train/summary_iterator][tf.train.summary_iterator]]: this is tf1.x API, but you can access it via
...
- [[https://stackoverflow.com/questions/37304461/tensorflow-importing-data-from-a-tensorboard-tfevent-file][this stackoverflow post]]: has a tf2 version:
