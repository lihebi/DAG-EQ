...
def mdn_nll(pi_mu_sigma, y, reduce=True):
    ...
    # d = torch.distributions.Normal(loc=torch.zeros(3), scale=torch.ones(3))
    # type(d)
    # d.sample()
    ...
    print(log_prob_y.size())
    print(log_prob_y)
    print(torch.mean(log_prob_y))
    log_prob_pi_y = log_prob_y + torch.log(pi)
    print(log_prob_pi_y.size())
    print(log_prob_pi_y)
    print(torch.mean(log_prob_pi_y))
    # loss = -torch.logsumexp(log_prob_pi_y, dim=1)
    loss = -torch.log(torch.sum(torch.exp(log_prob_pi_y), dim=1))
    print(loss.size())
    print(loss)
    if reduce:
...
def test_grad():
    # Tracker.gradient((mu, sigma) -> loglikelihood(Normal(mu, sigma), [8, 9]), 0, 1)
    def foo(mu, y):
        d = torch.distributions.Normal(loc=mu, scale=1)
        return torch.logsumexp(d.log_prob(y), dim=0)
    mu = torch.zeros(1)
    loss = foo(mu, torch.ones(3))
    loss.backward()
